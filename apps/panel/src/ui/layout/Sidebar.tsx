      ...
      >
        <div className={`sidebar-brand sidebar-brand--graphite flex h-28 shrink-0 flex-col justify-center gap-1 border-b border-white/10 px-5 transition-[padding] duration-300 ${sidebarCollapsed ? 'lg:items-center lg:px-2' : ''}`}>
          ...
          <button
            ...
            className={`absolute z-10 hidden rounded-lg border border-white/10 bg-white/5 p-1.5 text-white/70 backdrop-blur transition-all hover:border-accent/40 hover:bg-accent/15 hover:text-white lg:block ${sidebarCollapsed ? 'lg:right-1/2 lg:bottom-0.5 lg:translate-x-1/2' : 'top-2.5 right-2.5'}`}
          >
            <Menu className="h-4 w-4" />
          </button>
          <div className={`relative flex items-center transition-transform duration-300 ${sidebarCollapsed ? 'lg:-translate-y-3 lg:justify-center' : 'gap-3'}`}>
